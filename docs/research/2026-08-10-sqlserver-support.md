...
| 項目 | 決定 |
|---|---|
| SQL Server (.mdf/.bak) 対応 | **見送る。** Phase 0 も含め着手しない |
| 表示名の `database` → `Database` 大文字化 | **保留。** SQL Server 対応の方針が確定するまで実施しない |

見送りの背景は §5 のとおり。SQL Server は Express / Developer が無償で ATTACH / RESTORE でき、MDF ビューアの競合も多い。Oracle .dmp で成立している「ほぼ唯一の選択肢」というポジションは取れず、実働 3〜4 か月の投資対効果が Oracle 側の機能改善を上回る保証がない。

### 再検討する場合の入口

- **Phase 0（形式判定のみ、2〜3 日）から。** §7 のとおり単体で使い道があり、Phase 1 以降の土台としても捨てにならない。加えて次項のとおり**保守負担がほぼ発生しない**ため、実験として残置するコストが小さい。
- 着手前に **古い版（2000 / 2005 / 2008）の MDF 検体の入手目処**を立てること。最大の差別化要因である「版の壁」（§5）は、Docker では検証できない。
- **OrcaMDF が GPL-3.0 である点**（§4.1）を実装者に必ず伝えること。クリーンルーム実装の前提が崩れると後戻りできない。

### この投資は減価しない（見送り判断後に追記）

**MDF のページ構造とレコード形式は SQL Server 7 / 2005 以降ほぼ変わっていない。** OrcaMDF が 2013 年で更新を止めていながら現在も相応に動くのは、そのためである。

これは実装判断において無視できない性質である。上流コスト（§6 の 3〜4 か月）は変わらないが、**書いたコードが陳腐化しない**。バージョン差はシステム基底表のスキーマ（§2.2）に閉じており、ページ / レコードエンジン本体は一度書けば追随不要である。

Oracle 側と対照的である。あちらは EXP → EXPDP の二形式と Oracle 7 〜 23ai の版差を継続的に追ってきた。MDF にその追随コストはほぼ発生しない。

なお、**この性質は「OrcaMDF が放置されている＝空白がある」という理由づけとは別物である**（むしろ後者を弱める。放置されたツールが壊れて利用者が困っている、という構図が成立しないため）。商用の MDF ビューアは現役で複数存在し（§5）、空いているのは OSS の枠であって利用者から見た選択肢の枠ではない。再検討の際、見送り判断を覆す根拠として使えるのは投資の非減価性のほうであり、OrcaMDF の停滞ではない。

### 積み残しの論点

名称の大文字化は「SQL Server 対応の方針確定まで保留」として決定したが、その方針は本決定により「見送り」で確定している。**見送りが長期にわたるなら、大文字化を単独で実施する判断があり得る**（§8.6 末尾）。Oracle の正式表記に合わせる商標コンプライアンス上の是正であり、リポジトリ内 1 日・識別子への影響なしで完了する。再検討の際は §8 の実測値をそのまま使える。
...
[OrcaMDF](https://github.com/improvedk/OrcaMDF)（C#）は **GPL-3.0** である。本製品はプロプライエタリなので、コードの流用は当然できない。それだけでなく、実装を読みながら書くこと自体が派生物性の議論を招く。

**OrcaMDF の位置づけを誤解しないこと。** これは Microsoft とは無関係の個人プロジェクト（Mark S. Rasmussen 氏）であり、公式でも準公式でもない。作者自身が README で "highly experimental" / "purely prototypical" と述べており、対象は SQL Server 2008 R2、更新は 2013 年頃で止まっている（GitHub star 191）。**MDF を読むための「公式ツール」は存在しない**。Microsoft の公式な立場は「SQL Server に ATTACH せよ」であり、フォーマット自体が非公開である。

OrcaMDF の価値は「フォーマットの規模と難所の地図」および「実現可能性の証拠」であって、仕様の典拠ではない。§4.1 の制約を踏まえると、典拠として使うべきは後述の公開文献のほうであり、実務上もそちらが優る。
